...
static void ConvertFromTiles4BppCell(unsigned char *src, unsigned char *dest, int oamWidth, int oamHeight, int imageWidth, int startX, int startY, bool hFlip, bool vFlip, bool hvFlip, bool toPNG, int plttNum, UNUSED int mappingType, struct Image *image) {
    int tilesSoFar = 0;
    int rowsSoFar = 0;
    int chunkStartX = 0;
    int chunkStartY = 0;
    int pitch = image->hasPalette ? imageWidth / 2 : imageWidth;
            ...
            for (int k = 0; k < 4; k++) {
                ...
                if (hFlip) {
                    ...
                    if (toPNG) {
                        ...
                        if (image->hasPalette) {
                            dest[idxComponentY * pitch + idxComponentX * 2 + 0] = leftPixel | (plttNum << 4);
                            dest[idxComponentY * pitch + idxComponentX * 2 + 1] = rightPixel | (plttNum << 4);
                        } else {
                            dest[idxComponentY * pitch + idxComponentX] = (leftPixel << 4) | rightPixel;
                        }
                    } else {
                    ...
                    if (toPNG) {
                        if (image->hasPalette) {
                            ...
                            leftPixel = srcPixelPair & 0xF;
                            rightPixel = srcPixelPair >> 4;
                            dest[idxComponentY * pitch + idxComponentX * 2 + 0] = rightPixel | (plttNum << 4);
                            dest[idxComponentY * pitch + idxComponentX * 2 + 1] = leftPixel | (plttNum << 4);
                        } else {
...
static void ConvertFromTiles8BppCell(unsigned char *src, unsigned char *dest, int oamWidth, int oamHeight, int imageWidth, int startX, int startY, bool hFlip, bool vFlip, bool hvFlip, bool toPNG, int plttNum, int mappingType, struct Image *image) {
    ...
    int pitchFactor = mappingType == 2 ? (image->hasTransparency ? 4 : 3) : 1;
    pitch *= pitchFactor;
...
uint32_t ReadNtrImage(char *path, int tilesWide, int bitDepth, int colsPerChunk, int rowsPerChunk, struct Image *image, bool invertColors, bool scanFrontToBack) {
    ...
    if (scanned) {
        ...
        switch (bitDepth) {
        case 4:
            ConvertFromTiles4Bpp(imageData, image->pixels, numTiles, chunksWide, colsPerChunk, rowsPerChunk, invertColors);
            break;
        case 8:
            ConvertFromTiles8Bpp(imageData, image->pixels, numTiles, chunksWide, colsPerChunk, rowsPerChunk, invertColors);
            break;
        }
    }
...
void ApplyCellsToImage(char *cellFilePath, struct Image *image, bool toPNG) {
    ...
    uint32_t outSizeFactor_numer = 1, outSizeFactor_denom = 1;
    switch (options->mappingType) {
    case 0:
        if (image->bitDepth != 4) {
            FATAL_ERROR("cannot parse %dbpp image with mapping type 0\n", image->bitDepth);
        }
        if (!image->hasPalette) {
            outSizeFactor_denom = 2;
        }
        break;
    case 1:
        if (image->bitDepth != 8) {
    ...
    case 2:
        ...
        if (image->hasPalette) {
            outSizeFactor_numer = image->hasTransparency ? 4 : 3;
        }
        break;
    default:
        FATAL_ERROR("invalid mappingType %d\n", options->mappingType);
    }
    unsigned char *newPixels = malloc(outputHeight * outputWidth * outSizeFactor_numer / outSizeFactor_denom);
    memset(newPixels, image->hasPalette ? 0 : 255, outputHeight * outputWidth * outSizeFactor_numer / outSizeFactor_denom);
    ...
    for (int i = 0; i < options->cellCount; i++) {
        ...
        for (int j = 0; j < options->cells[i]->oamCount; j++) {
            ...
            case 4:
                ConvertFromTiles4BppCell(image->pixels + pixelOffset, newPixels, oamWidth, oamHeight, outputWidth, x, y + scanHeight, hFlip, vFlip, hvFlip, toPNG, plttNum, options->mappingType, image);
                break;
            case 8:
                pixelOffset *= 2;
                ConvertFromTiles8BppCell(image->pixels + pixelOffset, newPixels, oamWidth, oamHeight, outputWidth, x, y + scanHeight, hFlip, vFlip, hvFlip, toPNG, plttNum, options->mappingType, image);
                break;
            }
        }
...
void WriteNtrImage(char *path, int numTiles, int bitDepth, int colsPerChunk, int rowsPerChunk, struct Image *image, bool invertColors, bool clobberSize, bool byteOrder, bool version101, bool sopc, bool vram, uint32_t scanMode, uint32_t mappingType, uint32_t key, bool wrongSize) {
    ...
    if (scanMode) {
        ...
        switch (bitDepth) {
        case 4:
            ConvertToTiles4Bpp(image->pixels, pixelBuffer, numTiles, chunksWide, colsPerChunk, rowsPerChunk, invertColors);
            break;
        case 8:
            ConvertToTiles8Bpp(image->pixels, pixelBuffer, numTiles, chunksWide, colsPerChunk, rowsPerChunk, invertColors);
            break;
        }
    }
    ...
    unsigned char charHeader[0x20] = { 0x52, 0x41, 0x48, 0x43, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x00, 0x00, 0x00 };
...
void WriteNtrCell(char *path, struct JsonToCellOptions *options) {
    ...
    unsigned char KBECHeader[0x20] = {
        0x4B, 0x42, 0x45, 0x43, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
    };
...
void WriteNtrScreen(char *path, struct JsonToScreenOptions *options) {
    ...
    unsigned char NSCRHeader[0x14] = { 0x4E, 0x52, 0x43, 0x53, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
void WriteNtrAnimation(char *path, struct JsonToAnimationOptions *options) {
    ...
    unsigned char KBNAHeader[0x20] = {
        0x4B, 0x4E, 0x42, 0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
    };
