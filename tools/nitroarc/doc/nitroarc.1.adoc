...
*-c, --create*::
  ...
  _DIRECTORY_ or any of its subdirectories will be included in _ARCHIVE_. By
  default, all files which match either of the patterns “.\*ignore” or
  “.\*order” will be ignored. If a file named “.narcorder” is present in
  _DIRECTORY_, then it will be automatically loaded as if it had been given as
  an argument to the *-T* option. If a file named “.narcignore” is present in
  _DIRECTORY_, then it will be automatically loaded as if it had been given as
  an argument to the *-X* option. In either case, a file given as an explicit
  option-argument will take precedence.
...
*-P, --index-namespace*::
  Identical to *--index*, but all output preprocessor definitions are prefixed
  with a namespace of “NARC_” and the name of the output archive, minus its
  extension.
