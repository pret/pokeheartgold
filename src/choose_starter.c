...
void LaunchStarterChoiceScene(FieldSystem *fieldSystem) {
    struct ChooseStarterTaskData *env = AllocFromHeapAtEnd(HEAP_ID_FIELD2, sizeof(struct ChooseStarterTaskData));
    env->state = 0;
...
static BOOL CreateStarter(TaskManager *taskManager) {
    ...
    case 1:
        ...
        }
        {
            ...
            env->args = AllocFromHeapAtEnd(HEAP_ID_FIELD2, sizeof(struct ChooseStarterArgs));
