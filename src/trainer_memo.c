...
static void FormatDateAndLocationMet(Unk0208E600 *a0, int msgNo) {
    ...
    BufferIntegerAsString(a0->msgFmt, 5, Pokemon_GetData(a0->mon, MON_DATA_EGG_YEAR, NULL), 2, PRINTING_MODE_LEADING_ZEROS, TRUE);
    BufferMonthNameAbbr(a0->msgFmt, 6, Pokemon_GetData(a0->mon, MON_DATA_EGG_MONTH, NULL));
    BufferIntegerAsString(a0->msgFmt, 7, Pokemon_GetData(a0->mon, MON_DATA_EGG_DAY, NULL), 2, PRINTING_MODE_LEFT_ALIGN, TRUE);
    BufferLocationName(a0->msgFmt, 8, Pokemon_GetData(a0->mon, MON_DATA_EGG_LOCATION, NULL));
...
static void FormatFlavorPreference(Unk0208E600 *a0) {
    ...
    int index = 0;
    for (int flavor = FLAVOR_START; flavor < FLAVOR_COUNT; flavor++) {
...
static MetCondition MonMetCondition(Pokemon *mon, BOOL isMine) {
    if (!Pokemon_GetData(mon, MON_DATA_IS_EGG, NULL)) {
        if (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == 0) {
        ...
        if (Pokemon_GetData(mon, MON_DATA_FATEFUL_ENCOUNTER, NULL) == TRUE) {
            if (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_GIFT, MAPLOC(METLOC_LINK_TRADE_2))) {
                if (isMine == TRUE) {
                    return MET_CONDITION_FATEFUL_EGG_HATCHED_GIFT;
                }
                return MET_CONDITION_FATEFUL_EGG_HATCHED_GIFT_TRADED;
            }
            if (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_EXTERNAL, MAPLOC(METLOC_POKEMON_RANGER))) {
                if (isMine == TRUE) {
        ...
        if ((Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_GIFT, MAPLOC(METLOC_LINK_TRADE)))
            || (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_GIFT, MAPLOC(METLOC_DAY_CARE_COUPLE)))
            || (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_GIFT, MAPLOC(METLOC_TRAVELING_MAN)))
            || (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_GIFT, MAPLOC(METLOC_RILEY)))
            || (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_GIFT, MAPLOC(METLOC_CYNTHIA)))
            || (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_GIFT, MAPLOC(METLOC_MR_POKEMON)))
            || (Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL) == sub_02017FE4(MAPSECTYPE_GIFT, MAPLOC(METLOC_PRIMO)))) {
            if (isMine == TRUE) {
