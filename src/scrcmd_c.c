...
BOOL ScrCmd_563(ScriptContext *ctx) {
    ...
    cmd = AllocFromHeap(HEAP_ID_FIELD1, 64 * sizeof(MovementScriptCommand));
...
void _ScheduleObjectEventMovement(FieldSystem *fieldSystem, EventObjectMovementMan *movementMan, MovementScriptCommand *a2) {
    struct ObjectMovementTaskEnv *env = AllocFromHeap(HEAP_ID_FIELD1, sizeof(struct ObjectMovementTaskEnv));
    if (env == NULL) {
        GF_ASSERT(FALSE);
        return;
    }
...
BOOL ScrCmd_408(ScriptContext *ctx) {
    ...
    *p_work = AllocFromHeap(HEAP_ID_FIELD2, sizeof(struct UnkOv67Args));
...
BOOL ScrCmd_627(ScriptContext *ctx) {
    ...
    u8 r6 = ScriptReadByte(ctx);
    FrontierLaunchParam *param = AllocFromHeapAtEnd(HEAP_ID_FIELD2, sizeof(FrontierLaunchParam));
...
BOOL ScrCmd_682(ScriptContext *ctx) {
    ...
    u32 heap11Size = GF_ExpHeap_FndGetTotalFreeSize(HEAP_ID_FIELD2);
    u32 heap4Size = GF_ExpHeap_FndGetTotalFreeSize(HEAP_ID_FIELD1);
    u32 heap32Size = GF_ExpHeap_FndGetTotalFreeSize(HEAP_ID_FIELD3);
...
BOOL ScrCmd_BankTransaction(ScriptContext *ctx) {
    ...
    u16 mode = ScriptReadHalfword(ctx);
    u16 var_ret = ScriptReadHalfword(ctx);
    struct BankTransactionWork *work = *p_work = AllocFromHeap(HEAP_ID_FIELD1, sizeof(struct BankTransactionWork)); // statement must be this way to match
    work->sub = AllocFromHeap(HEAP_ID_FIELD1, sizeof(struct BankTransactionWorkSub));
    work->mode = mode;
