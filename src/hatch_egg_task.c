...
static BOOL Task_HatchEggInParty(TaskManager *taskManager) {
    ...
    case HATCHEGGTASKSTATE_LEAVE_OVERWORLD:
        sub_0206D328(data->unkC.mon, HEAP_ID_FIELDMAP);
    ...
    case HATCHEGGTASKSTATE_UPDATE_MON_INFO: {
        ...
        MonSetTrainerMemo(pokemon, profile, 6, mapsec, HEAP_ID_FIELDMAP);
    ...
    case HATCHEGGTASKSTATE_OPEN_NAMING_SCREEN: {
        ...
        data->namingScreenArgs = NamingScreen_CreateArgs(HEAP_ID_FIELDMAP, NAME_SCREEN_POKEMON, species, POKEMON_NAME_LENGTH, options, NULL);
...
void CallTask_HatchEggInParty(TaskManager *taskManager, UnkStruct_02091240 *a1) {
    HatchEggTaskData *data = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(HatchEggTaskData));
