...
static BOOL Task_HatchEggInParty(TaskManager *taskManager) {
    ...
    case HATCHEGGTASKSTATE_OPEN_NAMING_SCREEN: {
        ...
        u32 species = Pokemon_GetMonData(data->unkC.mon, MON_DATA_SPECIES, NULL);
        ...
        data->namingScreenArgs->monGender = Pokemon_GetMonData(data->unkC.mon, MON_DATA_GENDER, NULL);
        data->namingScreenArgs->monForm = Pokemon_GetMonData(data->unkC.mon, MON_DATA_FORM, NULL);
