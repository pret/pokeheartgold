...
TaskManager *CallFieldTask_ContinueGame_CommError(FieldSystem *fieldSystem) {
    ...
    env = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(struct ErrorContinueEnv));
    env->state = 0;
...
static void sub_02053710(TaskManager *taskManager, Location *location) {
    struct UnkTaskEnv_02053688 *env = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(struct UnkTaskEnv_02053688));
    env->unk0 = 0;
...
void sub_020537A8(TaskManager *taskManager, const Location *location) {
    FieldSystem *fieldSystem = TaskManager_GetFieldSystem(taskManager);
    struct ErrorContinueEnv *env = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(struct ErrorContinueEnv));
...
TaskManager *sub_020538C0(FieldSystem *fieldSystem, u32 mapId, int warpId, int x, int y, int direction) {
    ...
    struct UnkTaskEnv_02053950 *env = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(struct UnkTaskEnv_02053950));
    env->unk0 = 0;
    env->unk4 = 0;
    env->location = location;
...
void sub_02053B04(TaskManager *taskManager, Location *location, int a2) {
    struct UnkTaskEnv_02053B3C *env = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(struct UnkTaskEnv_02053B3C));
    env->unk0 = 0;
    env->unk4 = a2;
    env->unk8 = NULL;
    env->location = *location;
...
TaskManager *sub_02053E08(FieldSystem *fieldSystem, u32 mapId, int warpId) {
    ...
    env = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(struct UnkTaskEnv_02053CCC));
...
void sub_02053F14(FieldSystem *fieldSystem) {
    ...
    struct UnkTaskEnv_02053E5C *env = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(struct UnkTaskEnv_02053E5C));
...
void sub_0205412C(TaskManager *taskManager, u32 mapId, int warpId, int x, int y, int direction) {
    ...
    fieldSystem->unk70 = 3;
    env = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(struct UnkTaskEnv_02053688));
    {
        Location location = {
            mapId,
            warpId,
