...
#pragma unused(dat2)
    FieldSystem *fieldSystem = TaskManager_GetFieldSystem(data->taskManager);
    StartMenuTaskData *env = TaskManager_GetEnvironment(data->taskManager);
    PartyMenuArgs *usedat = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(PartyMenuArgs));
...
#pragma unused(dat2)
    FieldSystem *fieldSystem = TaskManager_GetFieldSystem(data->taskManager);
    StartMenuTaskData *env = TaskManager_GetEnvironment(data->taskManager);
    UseMailArgs *mailWork = UseMail_CreateArgs(fieldSystem, 3, ItemToMailId(data->itemId), HEAP_ID_FIELDMAP);
...
static void ItemMenuUseFunc_PalPad(struct ItemMenuUseData *data, const struct ItemCheckUseData *dat2) {
    FieldSystem *fieldSystem = TaskManager_GetFieldSystem(data->taskManager);
    StartMenuTaskData *env = TaskManager_GetEnvironment(data->taskManager);
    env->exitTaskEnvironment = PalPad_LaunchApp(fieldSystem, fieldSystem->saveData, HEAP_ID_FIELDMAP);
...
static PalPadArgs *_CreatePalPadArgs(FieldSystem *fieldSystem) {
    return PalPad_LaunchApp(fieldSystem, fieldSystem->saveData, HEAP_ID_FIELDMAP);
}
...
static void ItemMenuUseFunc_Honey(struct ItemMenuUseData *data, const struct ItemCheckUseData *dat2) {
    ...
    honey_work = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, size);
    memset(honey_work, 0, size);
    env->exitTaskFunc = Task_HoneyOrSweetScent;
    env->exitTaskEnvironment = honey_work;
    env->state = 12;
    Bag_TakeItem(Save_Bag_Get(fieldSystem->saveData), data->itemId, 1, HEAP_ID_FIELDMAP);
}
...
static void ItemMenuUseFunc_OldRod(struct ItemMenuUseData *data, const struct ItemCheckUseData *dat2) {
    ...
    env->exitTaskEnvironment = CreateFishingRodTaskEnv(fieldSystem, HEAP_ID_FIELDMAP, 0);
    env->state = 12;
}
...
static void ItemMenuUseFunc_GoodRod(struct ItemMenuUseData *data, const struct ItemCheckUseData *dat2) {
    ...
    FieldSystem_LoadFieldOverlay(fieldSystem);
    env->exitTaskFunc = Task_OverworldFish;
    env->exitTaskEnvironment = CreateFishingRodTaskEnv(fieldSystem, HEAP_ID_FIELDMAP, 1);
    env->state = 12;
}
...
static void ItemMenuUseFunc_SuperRod(struct ItemMenuUseData *data, const struct ItemCheckUseData *dat2) {
    ...
    FieldSystem_LoadFieldOverlay(fieldSystem);
    env->exitTaskFunc = Task_OverworldFish;
    env->exitTaskEnvironment = CreateFishingRodTaskEnv(fieldSystem, HEAP_ID_FIELDMAP, 2);
    env->state = 12;
}
...
static BOOL ItemFieldUseFunc_Generic(struct ItemFieldUseData *data) {
    struct RegisteredKeyItemUseMessagePrintTaskData *env = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(struct RegisteredKeyItemUseMessagePrintTaskData));
    env->state = 0;
    env->strbuf = String_New(128, HEAP_ID_FIELDMAP);
    TryFormatRegisteredKeyItemUseMessage(data->fieldSystem->saveData, env->strbuf, data->itemId, HEAP_ID_FIELDMAP);
...
static void ItemMenuUseFunc_EvoStone(struct ItemMenuUseData *data, const struct ItemCheckUseData *dat2) {
    ...
    usedat = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(PartyMenuArgs));
    memset(usedat, 0, sizeof(PartyMenuArgs));
    usedat->party = SaveArray_Party_Get(fieldSystem->saveData);
    usedat->bag = Save_Bag_Get(fieldSystem->saveData);
    usedat->mailbox = Save_Mailbox_Get(fieldSystem->saveData);
...
static BOOL Task_JumpToFieldEscapeRope(TaskManager *taskManager) {
    TaskManager_Jump(taskManager, Task_FieldEscapeRope, CreateFieldEscapeRopeTaskEnv(TaskManager_GetFieldSystem(taskManager), HEAP_ID_FIELDMAP));
    return FALSE;
}
...
static BOOL ItemFieldUseFunc_GbSounds(struct ItemFieldUseData *data) {
    struct RegisteredKeyItemUseMessagePrintTaskData *env = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(struct RegisteredKeyItemUseMessagePrintTaskData));
    env->state = 0;
    MsgData *msgData = NewMsgDataFromNarc(MSGDATA_LOAD_DIRECT, NARC_msgdata_msg, NARC_msg_msg_0010_bin, HEAP_ID_FIELDMAP);
...
static void ItemMenuUseFunc_Gracidea(struct ItemMenuUseData *data, const struct ItemCheckUseData *dat2) {
    FieldSystem *fieldSystem = TaskManager_GetFieldSystem(data->taskManager);
    StartMenuTaskData *env = TaskManager_GetEnvironment(data->taskManager);
    env->exitTaskEnvironment = PartyMenu_LaunchApp_Gracidea(fieldSystem, HEAP_ID_FIELDMAP, ITEM_GRACIDEA);
    StartMenu_SetExitTaskFunc(env, Task_StartMenu_HandleReturn_Pokemon);
}
...
static PartyMenuArgs *_CreateGracideaWork(FieldSystem *fieldSystem) {
    return PartyMenu_LaunchApp_Gracidea(fieldSystem, HEAP_ID_FIELDMAP, ITEM_GRACIDEA);
}
...
int UseRegisteredItemButtonInField(FieldSystem *fieldSystem, u8 slot) {
    ...
    funcType = GetItemAttr(itemId, ITEMATTR_FIELDUSEFUNC, HEAP_ID_FIELDMAP);
    ...
    data = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(struct ItemFieldUseData));
...
static void RegisteredItem_GoToPrintErrorTask(struct ItemFieldUseData *data, enum ItemUseError error) {
    ...
    env->state = 0;
    env->strbuf = String_New(128, HEAP_ID_FIELDMAP);
    GetItemUseErrorMessage(Save_PlayerData_GetProfile(data->fieldSystem->saveData), env->strbuf, data->itemId, error, HEAP_ID_FIELDMAP);
    FieldSystem_CreateTask(data->fieldSystem, Task_PrintRegisteredKeyItemUseMessage, env);
}
