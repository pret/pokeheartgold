...
BOOL ScrCmd_TrainerBattle(ScriptContext *ctx) {
    ...
    SetupAndStartTrainerBattle(ctx->taskman, var0, var1, followerTrainerNum, var2, var3, HEAP_ID_FIELDMAP, winFlag);
    return TRUE;
}
...
BOOL ScrCmd_MultiBattle(ScriptContext *ctx) {
    ...
    SetupAndStartTrainerBattle(ctx->taskman, var1, var2, var0, 0, var3, HEAP_ID_FIELDMAP, winFlag);
    return TRUE;
}
...
BOOL ScrCmd_223(ScriptContext *ctx) {
    u32 *winFlag = FieldSysGetAttrAddr(ctx->fieldSystem, SCRIPTENV_BATTLE_WIN_FLAG);
    SetupAndStartTrainerBattle(ctx->taskman, 1, 0, 0, 0, 0, HEAP_ID_FIELDMAP, winFlag);
    return TRUE;
}
