...
LocalMapObject *FollowMon_InitMapObject(MapObjectManager *mapObjectManager, int x, int y, int direction, u32 mapNo) {
    ...
    if (partyCount != 0) {
        ...
        if (FollowMon_GetPermissionBySpeciesAndMap(species, mapNo)) {
            ...
            int gender = Pokemon_GetMonData(mon, MON_DATA_GENDER, NULL); // must be int to match, even though gender is u8
            ...
            FieldSystem_UnkSub108_Set(fieldSystem->unk108, mon, species, Pokemon_GetMonData(mon, MON_DATA_PERSONALITY, NULL));
...
void FieldSystem_UnkSub108_AddMonMood(FieldSystemUnk108 *unk, s8 by) {
    ...
    mood = Pokemon_GetMonData(unk->mon, MON_DATA_MOOD, NULL);
    if (mood + by > 127) {
        mood = 127;
    } else if (mood + by < -127) {
        mood = -127;
...
s8 FieldSystem_UnkSub108_GetMonMood(FieldSystemUnk108 *unk) {
    ...
    return Pokemon_GetMonData(unk->mon, MON_DATA_MOOD, NULL);
}
...
void FieldSystem_UnkSub108_MoveMoodTowardsNeutral(FieldSystemUnk108 *a0) {
    ...
    mood = Pokemon_GetMonData(a0->mon, MON_DATA_MOOD, NULL);
    if (mood < 0) {
        mood++;
    } else if (mood > 0) {
        mood--;
