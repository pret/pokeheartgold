...
static void RegisterHallOfFame_IndivMonsScene_SetPicGfxAndPltt(RegisterHallOfFameData *data, RegisterHofMon *hofMon, u8 whichFacing, int picIdx) {
    ...
    PokepicTemplate_GetPokemonSpriteCharAndPlttNarcIds(&drawMonStruct, hofMon->mon, whichFacing);
...
static void RegisterHallOfFame_IndivMonsScene_PrintMonDetailsOnWindow(RegisterHallOfFameData *data, u16 monIdx, int windowIdx) {
    ...
    BoxPokemon *boxmon = Pokemon_GetBox(mon);
...
static void RegisterHallOfFame_GetPartyDetails(RegisterHallOfFameData *data) {
    ...
    for (i = 0; i < Party_GetCount(data->args->party); ++i) {
        ...
        encry = Pokemon_AcquireMonLock(pokemon);
        if (!Pokemon_GetMonData(pokemon, MON_DATA_IS_EGG, NULL)) {
            hofMon = &data->mons[data->numMons];
            hofMon->mon = pokemon;
            hofMon->species = Pokemon_GetMonData(pokemon, MON_DATA_SPECIES, NULL);
            hofMon->personality = Pokemon_GetMonData(pokemon, MON_DATA_PERSONALITY, NULL);
            hofMon->form = Pokemon_GetMonData(pokemon, MON_DATA_FORM, NULL);
            hofMon->gender = Pokemon_GetMonData(pokemon, MON_DATA_GENDER, NULL);
            hofMon->yOffset = GetMonPicHeightBySpeciesGenderForm(hofMon->species, hofMon->gender, 0, hofMon->form, hofMon->personality) + 8;
            hofMon->metLocation = Pokemon_GetMonData(pokemon, MON_DATA_MET_LOCATION, NULL);
            hofMon->level = Pokemon_GetMonData(pokemon, MON_DATA_LEVEL, NULL);
            hofMon->partyIndex = i;
        ...
        Pokemon_ReleaseMonLock(pokemon, encry);
    }
    NARC_Delete(narc);
}

static RegisterHallOfFame_MetLocationType RegisterHallOfFame_GetMetLocationType(RegisterHallOfFameData *data, Pokemon *pokemon, PlayerProfile *profile) {
    BOOL encry = Pokemon_AcquireMonLock(pokemon);
    RegisterHallOfFame_MetLocationType ret;
    int version = Pokemon_GetMonData(pokemon, MON_DATA_MET_GAME, NULL);
    ...
    } else if (Pokemon_GetMonData(pokemon, MON_DATA_FATEFUL_ENCOUNTER, NULL)) {
    ...
    } else if (PlayerProfile_GetTrainerID(profile) != Pokemon_GetMonData(pokemon, MON_DATA_OT_ID, NULL)) {
        ret = REGHOF_METLOC_TRADED;
    } else {
        PlayerName_FlatToString(profile, data->strbuf1);
        Pokemon_GetMonData(pokemon, MON_DATA_OT_NAME_STRING, data->strbuf2);
        if (String_Compare(data->strbuf1, data->strbuf2)) {
            ret = REGHOF_METLOC_TRADED;
        } else if (Pokemon_GetMonData(pokemon, MON_DATA_MET_LOCATION, NULL) >= METLOC_DAY_CARE_COUPLE) {
            ret = REGHOF_METLOC_FATEFUL;
        } else if (Pokemon_GetMonData(pokemon, MON_DATA_EGG_MONTH, NULL) == 0) {
