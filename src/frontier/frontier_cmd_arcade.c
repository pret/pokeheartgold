...
BOOL FrtCmd_187(FrontierContext *ctx) {
    ...
    if (arcadeData->unk13 == 0x11) {
        ...
        for (i = 0; i < monCnt; i++) {
            ...
            u32 level = Pokemon_GetMonData(mon, MON_DATA_LEVEL, 0);
...
static void GameBoardArgs_Set(GAME_BOARD_ARGS *args, ArcadeContext *data) {
    ...
    for (int i = 0; i < partyCnt; i++) {
        ...
        data->savedHp[i] = Pokemon_GetMonData(mon, MON_DATA_MAX_HP, NULL);
        data->savedAtk[i] = Pokemon_GetMonData(mon, MON_DATA_ATK, NULL);
        data->savedDef[i] = Pokemon_GetMonData(mon, MON_DATA_DEF, NULL);
        data->savedSpd[i] = Pokemon_GetMonData(mon, MON_DATA_SPEED, NULL);
        data->savedSpAtk[i] = Pokemon_GetMonData(mon, MON_DATA_SP_ATK, NULL);
        data->savedSpDef[i] = Pokemon_GetMonData(mon, MON_DATA_SP_DEF, NULL);
    }
}
...
BOOL FrtCmd_ArcadeAction(FrontierContext *ctx) {
    ...
    case 15:
        ...
        *out = Pokemon_GetMonData(mon, MON_DATA_HELD_ITEM, NULL);
        break;
    case 16:
        *out = arcadeCtx->unk13;
        break;
