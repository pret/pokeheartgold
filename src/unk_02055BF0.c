...
void NewFieldTransitionEnvironment(FieldSystem *fieldSystem, int mapID, int warpID, int x, int y, int dir, u32 transNo) {
    FieldTransitionEnvironment *fenv = AllocFromHeapAtEnd(HEAP_ID_FIELD2, sizeof(FieldTransitionEnvironment));
    fenv->state = 0;
...
BOOL sub_020562B0(TaskManager *man) {
    ...
    case 0:
        ...
        sub_0205F328(obj, 0);
        fenv->unk18 = AllocFromHeapAtEnd(HEAP_ID_FIELD2, sizeof(FieldEnvSubUnk18));
        fenv18 = fenv->unk18;
        fenv18->state = 0;
