...
void NewFieldTransitionEnvironment(FieldSystem *fieldSystem, int mapID, int warpID, int x, int y, int dir, u32 transNo) {
    FieldTransitionEnvironment *fenv = Heap_AllocAtEnd(HEAP_ID_FIELD, sizeof(FieldTransitionEnvironment));
    fenv->state = 0;
...
BOOL sub_020562B0(TaskManager *man) {
    ...
    case 0:
        ...
        sub_0205F328(obj, 0);
        fenv->unk18 = Heap_AllocAtEnd(HEAP_ID_FIELD, sizeof(FieldEnvSubUnk18));
        fenv18 = fenv->unk18;
        fenv18->state = 0;
