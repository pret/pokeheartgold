...
void sub_0200CE7C(MessagePrinter *msgPrinter, u8 glyphId, u32 num, u32 ndigits, enum PaddingMode mode, Window *window, u32 x, u32 y) {
    sub_0200CDAC(msgPrinter, glyphId, window, x, y);
    PrintUIntOnWindow(msgPrinter, num, ndigits, mode, window, x + 16, y);
}

void sub_0200CEB0(BattleNumberPrinter *unk, u32 num, u32 ndigits, enum PaddingMode paddingMode, u16 *dest) {
    u8 fillVal = (unk->fillVal << 4) | unk->fillVal;
    ConvertUIntToDecimalString(unk->strbuf, num, paddingMode, ndigits);
