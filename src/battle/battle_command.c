...
enum {
    ...
    STATE_GET_EXP_CHECK_LEVEL_UP, //5
    ...
    STATE_GET_EXP_LEVEL_UP_SUMMARY_PRINT_DIFF, //10
...
static void Task_GetExp(SysTask *task, void *inData)
{
    ...
    battler = data->ctx->battlerIdFainted >> 1 & 1; // init to the side with the fainted mon
    expBattler = 0;
    ...
    for (slot = data->unk30[6]; slot < BattleSystem_GetPartySize(data->bsys, expBattler); slot++) {
        ...
        if (itemEffect == HOLD_EFFECT_EXP_SHARE || (data->ctx->unk_A4[battler] & MaskOfFlagNo(slot))) {
            break;
        }
    }
