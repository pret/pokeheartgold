...
void BattleSystem_GetBattleMon(BattleSystem *battleSystem, BattleContext *ctx, int battlerId, u8 selectedMon) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        ctx->battleMons[battlerId].movePPCur[i] = Pokemon_GetData(mon, MON_DATA_MOVE1_PP + i, NULL);
    ...
    ctx->battleMons[battlerId].otGender = Pokemon_GetData(mon, MON_DATA_OT_GENDER, NULL);
    ...
    Pokemon_GetData(mon, MON_DATA_NICKNAME, ctx->battleMons[battlerId].nickname);
...
BOOL ov12_022568B0(BattleSystem *battleSystem, Pokemon *mon) {
    ...
    if (trainerId == Pokemon_GetData(mon, MON_DATA_OT_ID, NULL) && gender == Pokemon_GetData(mon, MON_DATA_OT_GENDER, NULL) && !StringNotEqualN(name, otName, PLAYER_NAME_LENGTH)) {
        return TRUE;
    }
