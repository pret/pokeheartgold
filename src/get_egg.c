...
static void sub_0206D038(Pokemon *mon, HeapID heapId) {
    ...
    metYear = GetMonData(mon, MON_DATA_HATCH_YEAR, NULL);
    metMonth = GetMonData(mon, MON_DATA_HATCH_MONTH, NULL);
    metDay = GetMonData(mon, MON_DATA_HATCH_DAY, NULL);
    SetMonData(tmpMon, MON_DATA_HATCH_LOCATION, &metLoc);
    SetMonData(tmpMon, MON_DATA_HATCH_YEAR, &metYear);
    SetMonData(tmpMon, MON_DATA_HATCH_MONTH, &metMonth);
    SetMonData(tmpMon, MON_DATA_HATCH_DAY, &metDay);
