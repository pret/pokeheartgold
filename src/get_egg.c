...
static void Daycare_LearnLevelUpMoves(Pokemon *mon) {
    ...
    for (i = 0, last_i = 0; i < MAX_LEVEL; i++) {
        ...
        last_i = 0;
        for (
            stat = Pokemon_TryLevelUpMove(mon, &last_i, &learned);
            stat != 0;
            stat = Pokemon_TryLevelUpMove(mon, &last_i, &learned)) {
            if (stat == MOVE_APPEND_FULL) {
                Pokemon_ForceAppendMove(mon, learned);
            }
        }
    }
        ...
        for (j = 0; j < learnset_size; j++) {
            if (search->baby_learnset[j] != MOVE_NONE) {
                if (search->shared_moves[i] == search->baby_learnset[j]) {
                    if (Pokemon_TryAppendMove(egg, search->shared_moves[i]) == MOVE_APPEND_FULL) {
                        Pokemon_ForceAppendMove(egg, search->shared_moves[i]);
                    }
                    break;
                }
            }
...
static void Daycare_LightBallCheck(Pokemon *egg, Daycare *dayCare) {
    ...
    if (item1 == ITEM_LIGHT_BALL || item2 == ITEM_LIGHT_BALL) {
        if (Pokemon_TryAppendMove(egg, MOVE_VOLT_TACKLE) == MOVE_APPEND_FULL) {
            Pokemon_ForceAppendMove(egg, MOVE_VOLT_TACKLE);
        }
    }
}
...
void SetEggStats(Pokemon *mon, int species, u8 metLocation, PlayerProfile *profile, int trainerMemoStrat, int eggLocation) {
    ...
    if (metLocation != MAPSEC_MYSTERY_ZONE) {
        Pokemon_SetData(mon, MON_DATA_EGG_LOCATION, &metLocation);
    }
    isEgg = TRUE;
...
static void sub_0206D038(Pokemon *mon, enum HeapID heapID) {
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) {
        ...
        pp[i] = Pokemon_GetData(mon, MON_DATA_MOVE1_PP + i, NULL);
    }
    ...
    otGender = Pokemon_GetData(mon, MON_DATA_OT_GENDER, NULL);
    ...
    metLoc = Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL);
    metYear = Pokemon_GetData(mon, MON_DATA_EGG_YEAR, NULL);
    metMonth = Pokemon_GetData(mon, MON_DATA_EGG_MONTH, NULL);
    metDay = Pokemon_GetData(mon, MON_DATA_EGG_DAY, NULL);
