...
static StartMenuTaskData *StartMenu_Create(void) {
    StartMenuTaskData *ret = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(StartMenuTaskData));
...
static void Task_StartMenu_DrawCursor(TaskManager *taskManager) {
    ...
    GfGfxLoader_LoadCharData(NARC_a_0_1_4, 12, fieldSystem->bgConfig, GF_BG_LYR_MAIN_3, 0, 0, TRUE, HEAP_ID_FIELDMAP);
    GfGfxLoader_GXLoadPal(NARC_a_0_1_4, 15, GF_PAL_LOCATION_MAIN_BG, (enum GFPalSlotOffset)0x1C0, 0x20, HEAP_ID_FIELDMAP);
    GfGfxLoader_LoadScrnData(NARC_a_0_1_4, 13, fieldSystem->bgConfig, GF_BG_LYR_MAIN_3, 0, 0, TRUE, HEAP_ID_FIELDMAP);
...
static void StartMenu_CreateCursor(StartMenuTaskData *startMenu, u8 *a1, u32 a2, u8 gender) {
    startMenu->spriteList = G2dRenderer_Init(1, &startMenu->g2dRenderer, HEAP_ID_FIELDMAP);
    for (int i = 0; i < 4; ++i) {
        startMenu->gfxResMan[i] = Create2DGfxResObjMan(1, (GfGfxResType)i, HEAP_ID_FIELDMAP);
    }
    startMenu->gfxResObj[GF_GFX_RES_TYPE_CHAR] = AddCharResObjFromNarc(startMenu->gfxResMan[GF_GFX_RES_TYPE_CHAR], NARC_a_0_1_4, 64, TRUE, 0, NNS_G2D_VRAM_TYPE_2DMAIN, HEAP_ID_FIELDMAP);
    startMenu->gfxResObj[GF_GFX_RES_TYPE_PLTT] = AddPlttResObjFromNarc(startMenu->gfxResMan[GF_GFX_RES_TYPE_PLTT], NARC_a_0_1_4, 61, FALSE, 0, NNS_G2D_VRAM_TYPE_2DMAIN, 1, HEAP_ID_FIELDMAP);
    startMenu->gfxResObj[GF_GFX_RES_TYPE_CELL] = AddCellOrAnimResObjFromNarc(startMenu->gfxResMan[GF_GFX_RES_TYPE_CELL], NARC_a_0_1_4, 62, TRUE, 0, GF_GFX_RES_TYPE_CELL, HEAP_ID_FIELDMAP);
    startMenu->gfxResObj[GF_GFX_RES_TYPE_ANIM] = AddCellOrAnimResObjFromNarc(startMenu->gfxResMan[GF_GFX_RES_TYPE_ANIM], NARC_a_0_1_4, 63, TRUE, 0, GF_GFX_RES_TYPE_ANIM, HEAP_ID_FIELDMAP);
    ...
    spriteTemplate.heapId = HEAP_ID_FIELDMAP;
...
static BOOL Task_StartMenu_OpenPokedex(TaskManager *taskManager) {
    ...
    PokedexArgs *pokedexArgs = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(PokedexArgs));
...
static BOOL Task_StartMenu_HandleReturn_Pokedex(TaskManager *taskManager) {
    ...
    if (startMenu->exitTaskEnvironment != NULL) {
        FreeToHeapExplicit(HEAP_ID_FIELDMAP, startMenu->exitTaskEnvironment);
    }
...
BOOL Task_StartMenu_HandleReturn_Pokemon(TaskManager *taskManager) {
    ...
    PartyMenuArgs *partyMenuArgs = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(PartyMenuArgs));
    ...
    case PARTY_MENU_ACTION_RETURN_1: {
        PokemonSummaryArgs *pokemonSummaryArgs = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(PokemonSummaryArgs));
    ...
    case PARTY_MENU_ACTION_RETURN_4: {
        ...
        sub_0208AD34(pokemonSummaryArgs, Save_PlayerData_GetProfile(fieldSystem->saveData));
        PokemonSummary_LearnForget_LaunchApp(fieldSystem, pokemonSummaryArgs);
        UnkStruct_0203D580 *unk = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(UnkStruct_0203D580));
    ...
    case PARTY_MENU_ACTION_RETURN_6:
        startMenu->exitTaskEnvironment = sub_0203EFEC(fieldSystem, 2, partyMenuArgs->partySlot, ItemToMailId(partyMenuArgs->itemId), HEAP_ID_FIELDMAP);
    ...
    case PARTY_MENU_ACTION_RETURN_READ_MAIL:
        startMenu->exitTaskEnvironment = sub_0203F050(fieldSystem, Party_GetMonByIndex(SaveArray_Party_Get(fieldSystem->saveData), partyMenuArgs->partySlot), HEAP_ID_FIELDMAP);
        startMenu->exitTaskEnvironment2 = sub_0203D818(partyMenuArgs->itemId, 2, partyMenuArgs->partySlot);
        StartMenu_SetExitTaskFunc(startMenu, Task_ReturnToMenuFromMail);
        break;
    case 3: {
        StartMenuAfterEvoPartySlotBak *afterEvoPartySlot = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(StartMenuAfterEvoPartySlotBak));
        ...
        (void)playerProfile;
        startMenu->exitTaskEnvironment = Bag_CreateView(bag, sPockets, HEAP_ID_FIELDMAP);
    ...
    case PARTY_MENU_ACTION_RETURN_EVO_ITEM_USE: {
        UnkStruct_0203CA9C_Case8 *unk = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(UnkStruct_0203CA9C_Case8));
...
static BOOL Task_StartMenu_HandleReturn(TaskManager *taskManager) {
    ...
    BagView *bagView = BagView_New(HEAP_ID_FIELDMAP);
    ...
    case 0: {
        ...
        ItemMenuUseFunc func = GetItemFieldUseFunc(USE_ITEM_TASK_MENU, GetItemAttr(itemMenuUseData.itemId, ITEMATTR_FIELDUSEFUNC, HEAP_ID_FIELDMAP));
    ...
    case 4: {
        ...
        if (ItemIdIsMail(itemId) == TRUE && !GetMonData(pokemon, MON_DATA_HELD_ITEM, NULL)) {
            startMenu->exitTaskEnvironment = sub_0203EFEC(fieldSystem, 2, monSlot, ItemToMailId(itemId), HEAP_ID_FIELDMAP);
...
static BOOL sub_0203D1CC(TaskManager *taskManager) {
    ...
    startMenu->exitTaskEnvironment = sub_020691C4(HEAP_ID_FIELDMAP);
...
static BOOL Task_StartMenu_RemovedEasyChatThing(TaskManager *taskManager) {
    ...
    startMenu->exitTaskEnvironment = EasyChat_CreateArgs(2, 0, fieldSystem->saveData, &fieldSystem->menuInputState, HEAP_ID_FIELDMAP);
...
static BOOL sub_0203D580(TaskManager *taskManager) {
    ...
    PokemonSummaryArgs *summaryArgs = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(PokemonSummaryArgs));
...
static void sub_0203D664(TaskManager *taskManager, int a1) {
    ...
    for (u8 i = 0; i < NUM_BERRIES; ++i) {
        if (Bag_HasItem(bag, BerryToItemId(i), 1, HEAP_ID_FIELDMAP)) {}
    }
    u8 scroll, position;
...
BOOL Task_UseFlyInField(TaskManager *taskManager) {
    ...
    FreeToHeapExplicit(HEAP_ID_FIELDMAP, flyMap);
    PokegearArgs *pokegearArgs = startMenu->exitTaskEnvironment;
    if (!pokegearArgs->unk_14) {
        FreeToHeapExplicit(HEAP_ID_FIELDMAP, pokegearArgs);
        ...
        struct UnkStruct_02067BF8 *r5 = sub_02067BF8(HEAP_ID_FIELDMAP, fieldSystem, pokemon, partySlot, pokegearArgs->unk_20, pokegearArgs->mapCursorX * 32 + 0x10, pokegearArgs->mapCursorY * 32 + 0x10);
        FreeToHeapExplicit(HEAP_ID_FIELDMAP, startMenu->exitTaskEnvironment);
        FieldSystem_LoadFieldOverlay(fieldSystem);
...
UnkStruct_0203D818 *sub_0203D818(u16 itemId, u8 kind, u8 partySlot) {
    UnkStruct_0203D818 *ret = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(UnkStruct_0203D818));
    ret->itemId = itemId;
...
static void Task_StartMenu_Evolution(TaskManager *taskManager) {
    ...
    StartMenuAfterEvoPartySlotBak *newEnv = AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(StartMenuAfterEvoPartySlotBak));
