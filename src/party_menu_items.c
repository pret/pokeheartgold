...
static void PartyMenu_GetItemUseMessage(PartyMenu *partyMenu, u16 itemId, int param) {
    ...
    BufferBoxMonNickname(partyMenu->msgFormat, 0, Pokemon_GetBox(Party_GetMonByIndex(partyMenu->args->party, partyMenu->partyMonIndex)));
...
static int PartyMenu_ItemUseFunc_EVDown(PartyMenu *partyMenu) {
    ...
    u8 hpEv = Pokemon_GetMonData(mon, MON_DATA_HP_EV, NULL);
    u8 atkEv = Pokemon_GetMonData(mon, MON_DATA_ATK_EV, NULL);
    u8 defEv = Pokemon_GetMonData(mon, MON_DATA_DEF_EV, NULL);
    u8 speedEv = Pokemon_GetMonData(mon, MON_DATA_SPEED_EV, NULL);
    u8 spAtkEv = Pokemon_GetMonData(mon, MON_DATA_SPATK_EV, NULL);
    u8 spDefEv = Pokemon_GetMonData(mon, MON_DATA_SPDEF_EV, NULL);
    u8 friendship = Pokemon_GetMonData(mon, MON_DATA_FRIENDSHIP, NULL);
    UseItemOnMonInParty(partyMenu->args->party, partyMenu->args->itemId, partyMenu->partyMonIndex, 0, PartyMenu_GetCurrentMapSec(partyMenu), HEAP_ID_PARTY_MENU);
    sub_02079E38(partyMenu, partyMenu->partyMonIndex);
    sub_0207D5DC(partyMenu, partyMenu->partyMonIndex);
    PartyMenu_CommitPartyMonPanelWindowsToVram_InVBlank(partyMenu, partyMenu->partyMonIndex);
    PartyMenu_DrawMonStatusIcon(partyMenu, partyMenu->partyMonIndex, partyMenu->monsDrawState[partyMenu->partyMonIndex].status);
    if (
        hpEv != Pokemon_GetMonData(mon, MON_DATA_HP_EV, NULL) || atkEv != Pokemon_GetMonData(mon, MON_DATA_ATK_EV, NULL) || defEv != Pokemon_GetMonData(mon, MON_DATA_DEF_EV, NULL) || speedEv != Pokemon_GetMonData(mon, MON_DATA_SPEED_EV, NULL) || spAtkEv != Pokemon_GetMonData(mon, MON_DATA_SPATK_EV, NULL) || spDefEv != Pokemon_GetMonData(mon, MON_DATA_SPDEF_EV, NULL)) {
        if (friendship != Pokemon_GetMonData(mon, MON_DATA_FRIENDSHIP, NULL)) {
            PartyMenu_GetItemUseMessage(partyMenu, partyMenu->args->itemId, 0);
        } else {
...
static int PartyMenu_ItemUseFunc_HPRestoreAnimLoop(PartyMenu *partyMenu) {
    int hp = Pokemon_GetMonData(Party_GetMonByIndex(partyMenu->args->party, partyMenu->partyMonIndex), MON_DATA_HP, NULL);
...
static int PartyMenu_ItemUseFunc_LevelUp(PartyMenu *partyMenu) {
    Pokemon *mon = Party_GetMonByIndex(partyMenu->args->party, partyMenu->partyMonIndex);
    partyMenu->levelUpStatsTmp[0] = Pokemon_GetMonData(mon, MON_DATA_MAX_HP, NULL);
    partyMenu->levelUpStatsTmp[1] = Pokemon_GetMonData(mon, MON_DATA_ATK, NULL);
    partyMenu->levelUpStatsTmp[2] = Pokemon_GetMonData(mon, MON_DATA_DEF, NULL);
    partyMenu->levelUpStatsTmp[3] = Pokemon_GetMonData(mon, MON_DATA_SP_ATK, NULL);
    partyMenu->levelUpStatsTmp[4] = Pokemon_GetMonData(mon, MON_DATA_SP_DEF, NULL);
    partyMenu->levelUpStatsTmp[5] = Pokemon_GetMonData(mon, MON_DATA_SPEED, NULL);
    UseItemOnMonInParty(partyMenu->args->party, partyMenu->args->itemId, partyMenu->partyMonIndex, 0, PartyMenu_GetCurrentMapSec(partyMenu), HEAP_ID_PARTY_MENU);
    partyMenu->monsDrawState[partyMenu->partyMonIndex].level = Pokemon_GetMonData(mon, MON_DATA_LEVEL, NULL);
    partyMenu->monsDrawState[partyMenu->partyMonIndex].hp = Pokemon_GetMonData(mon, MON_DATA_HP, NULL);
    partyMenu->monsDrawState[partyMenu->partyMonIndex].maxHp = Pokemon_GetMonData(mon, MON_DATA_MAX_HP, NULL);
...
static int PartyMenu_ItemUseFunc_LevelUpLearnMovesLoop(PartyMenu *partyMenu) {
    ...
    case 3:
        mon = Party_GetMonByIndex(partyMenu->args->party, partyMenu->partyMonIndex);
        switch (Pokemon_TryLearnMoveOnLevelUp(mon, &partyMenu->args->levelUpMoveSearchState, &partyMenu->args->moveId)) {
        case MOVE_NONE:
    ...
    case 6:
        ...
        partyMenu->args->species = Pokemon_GetEvolution(partyMenu->args->party, mon, EVOCTX_LEVELUP, mapEvoMethod, &partyMenu->args->evoMethod);
...
int PartyMenu_ItemUseFunc_LevelUpDoLearnMove(PartyMenu *partyMenu) {
    ...
    if (partyMenu->args->selectedMoveIdx == 4) {
        ...
        BufferMoveName(partyMenu->msgFormat, 1, Pokemon_GetMonData(mon, MON_DATA_MOVE1 + partyMenu->args->selectedMoveIdx, NULL));
...
u8 PartyMenu_CheckCanLearnTMHMMove(PartyMenu *partyMenu, Pokemon *mon) {
    ...
    for (i = 0; i < MAX_MON_MOVES; ++i) {
        u16 move = Pokemon_GetMonData(mon, MON_DATA_MOVE1 + i, NULL);
    ...
    if (!Pokemon_GetTMHMCompat(mon, ItemToTMHMId(partyMenu->args->itemId))) {
...
void PartyMenu_LearnMoveToSlot(PartyMenu *partyMenu, Pokemon *mon, int moveIdx) {
    ...
    if (partyMenu->args->itemId != ITEM_NONE) {
        ...
        Pokemon_ApplyFriendshipMod(mon, FRIENDSHIP_EVENT_LEARN_TMHM, PartyMenu_GetCurrentMapSec(partyMenu));
...
int PartyMenu_Subtask_SelectMove(PartyMenu *partyMenu) {
    ...
    default:
        ...
        if (UseItemOnMonInParty(partyMenu->args->party, partyMenu->args->itemId, partyMenu->partyMonIndex, input, PartyMenu_GetCurrentMapSec(partyMenu), HEAP_ID_PARTY_MENU) == TRUE) {
            Pokemon *mon = Party_GetMonByIndex(partyMenu->args->party, partyMenu->partyMonIndex);
            int moveId = Pokemon_GetMonData(mon, MON_DATA_MOVE1 + input, NULL);
