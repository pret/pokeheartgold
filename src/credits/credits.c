...
BOOL CreditsApp_OvyInit(OVY_MANAGER *man, int *state) {
    ...
    case 1:
        ...
        AddTextPrinterParameterizedWithColor(&work->pageWork.window, 0, work->pageWork.string, 0, 70, TEXT_SPEED_NOTRANSFER, 0x00010200, NULL);
...
static void LoadPage(PageWork *ptr) {
    ...
    if (cur_lines_page == nextPage) {
        do {
            ...
            AddTextPrinterParameterizedWithColor(&ptr->window, 0, ptr->string, xPos, yPos, TEXT_SPEED_NOTRANSFER, 0x00010200, NULL);
