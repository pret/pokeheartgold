...
BOOL sub_0204AA78(UnkStruct_Fsys_A0 *a0, void **a1, SaveData *saveData) {
    ...
    for (u16 i = 0; i < a0->unk0e; i++) {
        ...
        a0->unk2e[i] = Pokemon_GetMonData(mon, MON_DATA_SPECIES, NULL);
        a0->unk36[i] = Pokemon_GetMonData(mon, MON_DATA_HELD_ITEM, NULL);
    }
    Heap_Free(*a1);
    *a1 = NULL;
    return TRUE;
...
u32 sub_0204AB10(UnkStruct_Fsys_A0 *a0, SaveData *saveData) {
    ...
    for (u16 i = 0; i < a0->unk0e; i++) {
        Pokemon *mon = Party_GetMonByIndex(party, a0->unk2a[i]);
        species[i] = Pokemon_GetMonData(mon, MON_DATA_SPECIES, NULL);
        items[i] = Pokemon_GetMonData(mon, MON_DATA_HELD_ITEM, NULL);
        if (i == 0) {
            continue;
        }
        for (u16 j = 0; j < i; j++) {
...
static void sub_0204B34C(UnkStruct_0204B470 *a0, Pokemon *mon) {
    a0->species = Pokemon_GetMonData(mon, MON_DATA_SPECIES, NULL);
    a0->form = Pokemon_GetMonData(mon, MON_DATA_FORM, NULL);
    a0->item = Pokemon_GetMonData(mon, MON_DATA_HELD_ITEM, NULL);
    for (s32 i = 0; i < MAX_MON_MOVES; i++) {
        a0->moves[i] = Pokemon_GetMonData(mon, MON_DATA_MOVE1 + i, NULL);
        a0->ppUp |= Pokemon_GetMonData(mon, MON_DATA_MOVE1_PP_UPS + i, NULL) << (i * 2);
    }
    a0->language = Pokemon_GetMonData(mon, MON_DATA_LANGUAGE, NULL);
    a0->otId = Pokemon_GetMonData(mon, MON_DATA_OT_ID, NULL);
    a0->pid = Pokemon_GetMonData(mon, MON_DATA_PERSONALITY, NULL);
    a0->ivsWord = Pokemon_GetMonData(mon, MON_DATA_COMBINED_IVS, NULL);
    for (s32 i = 0; i < NUM_STATS; i++) {
        a0->evs[i] = Pokemon_GetMonData(mon, MON_DATA_HP_EV + i, NULL);
    }
    a0->ability = Pokemon_GetMonData(mon, MON_DATA_ABILITY, NULL);
    a0->friendship = Pokemon_GetMonData(mon, MON_DATA_FRIENDSHIP, NULL);
    Pokemon_GetMonData(mon, MON_DATA_NICKNAME, a0->nickname);
}
