...
static void FrontierFieldSystem_SetFrontierMons(FrontierFieldSystem *frontierFsys, SaveData *saveData, BOOL isWifiRoom);
...
FrontierFieldSystem *FrontierFieldSystem_New(SaveData *saveData, BOOL resumeFromPrevious, u32 towerMode) {
    ...
    frontierFsys->initMagicNumber = 0x12345678;
    ...
    if (!resumeFromPrevious) {
        ...
        u8 data = frontierFsys->towerMode;
        sub_0202D308(frontierFsys->unk70, 0, &data);
    } else {
...
void FrontierFieldSystem_Free(FrontierFieldSystem *frontierFsys) {
    ...
    GF_ASSERT(frontierFsys->initMagicNumber == 0x12345678);
    MI_CpuClear8(frontierFsys, sizeof(FrontierFieldSystem));
...
void FrontierFieldSystem_SetRandomFrontierTrainers(FrontierFieldSystem *frontierFsys, SaveData *saveData) {
    ...
    if (frontierFsys->towerMode == TOWER_MODE_MULTI || frontierFsys->towerMode == TOWER_MODE_6 || frontierFsys->towerMode == TOWER_MODE_LINK) {
        ...
        if ((frontierFsys->towerMode == TOWER_MODE_LINK && frontierFsys->linkAllyRound > frontierFsys->currentRound)
            ...
            round = frontierFsys->linkAllyRound;
        } else {
            round = frontierFsys->currentRound;
        }
        difficultyBracket = round;
        for (i = 0; i < 14; i++) {
            do {
...
static void sub_0204ACA0(FrontierFieldSystem *frontierFsys, SaveData *saveData, BOOL unused_2, u16 unused_3) {
    switch (frontierFsys->towerMode) {
    case TOWER_MODE_SINGLE:
        FrontierFieldSystem_SetFrontierMons(frontierFsys, saveData, FALSE);
        break;
    case TOWER_MODE_DOUBLE:
        break;
    case TOWER_MODE_WIFI:
        FrontierFieldSystem_SetFrontierMons(frontierFsys, saveData, TRUE);
        sub_0202D3B0(frontierFsys->unk70, frontierFsys->unk28, frontierFsys->unk24, frontierFsys->unk26);
        u8 data = frontierFsys->towerMode;
        sub_0202D308(frontierFsys->unk70, 0, &data);
        data = frontierFsys->currentBattleNumber;
        sub_0202D308(frontierFsys->unk70, 1, &data);
...
void FrontierFieldSystem_0204AF2C(FrontierFieldSystem *frontierFsys) {
    ...
    if (frontierFsys->towerMode == TOWER_MODE_MULTI) {
        data = frontierFsys->multiBattleAllyID;
        sub_0202D308(frontierFsys->unk70, 9, &data);
...
u32 FrontierFieldSystem_0204B044(FrontierFieldSystem *frontierFsys, u32 a1) {
    GetFrontierTrainerOverworld(frontierFsys->unk78[a1].trainerClass);
}
...
static BOOL TryGiveTowerRibbons(SaveData *saveData, u32 ribbon, FrontierFieldSystem *a2) {
    u8 value = TRUE;
    Party *party = SaveArray_Party_Get(saveData);
    u8 count = 0;
    for (s32 i = 0; i < a2->numMons; i++) {
        Pokemon *mon = Party_GetMonByIndex(party, a2->partyMonIndexes[i]);
...
static void FrontierFieldSystem_SetFrontierMons(FrontierFieldSystem *frontierFsys, SaveData *saveData, BOOL isWifiRoom) {
    ...
    sub_0202D4B8(frontierFsys->frontierData, isWifiRoom, frontierMons); // FrontierData_SetFrontierMons?
    MI_CpuClear8(frontierMons, 3 * sizeof(FrontierMon));
