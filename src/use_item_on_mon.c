...
BOOL CanUseItemOnPokemon(Pokemon *mon, u16 itemID, s32 moveIdx, enum HeapID heapID) {
    ...
    u32 status = Pokemon_GetMonData(mon, MON_DATA_STATUS, NULL);
    // Sleep
    ...
    u32 hp = Pokemon_GetMonData(mon, MON_DATA_HP, NULL);
    ...
    } else if (GetItemAttr_PreloadedItemData(itemData, ITEMATTR_HP_RESTORE) && hp != 0 && hp < Pokemon_GetMonData(mon, MON_DATA_MAX_HP, NULL)) {
        Heap_Free(itemData);
        return TRUE;
    }

    if (GetItemAttr_PreloadedItemData(itemData, ITEMATTR_LEVEL_UP) && Pokemon_GetMonData(mon, MON_DATA_LEVEL, NULL) < MAX_LEVEL) {
        Heap_Free(itemData);
        return TRUE;
    }

    if (GetItemAttr_PreloadedItemData(itemData, ITEMATTR_EVOLVE) && Pokemon_GetEvolution(NULL, mon, EVOCTX_ITEM_USE, itemID, NULL) != SPECIES_NONE) {
        Heap_Free(itemData);
        return TRUE;
    }

    if (GetItemAttr_PreloadedItemData(itemData, ITEMATTR_PP_UP) || GetItemAttr_PreloadedItemData(itemData, ITEMATTR_PP_MAX)) {
        if (Pokemon_GetMonData(mon, MON_DATA_MOVE1_PP_UPS + moveIdx, NULL) < 3 && GetMoveMaxPP(Pokemon_GetMonData(mon, MON_DATA_MOVE1 + moveIdx, NULL), 0) >= 5) {
            Heap_Free(itemData);
            return TRUE;
        }
    }
    ...
    hpEv = Pokemon_GetMonData(mon, MON_DATA_HP_EV, NULL);
    atkEv = Pokemon_GetMonData(mon, MON_DATA_ATK_EV, NULL);
    defEv = Pokemon_GetMonData(mon, MON_DATA_DEF_EV, NULL);
    speedEv = Pokemon_GetMonData(mon, MON_DATA_SPEED_EV, NULL);
    spAtkEv = Pokemon_GetMonData(mon, MON_DATA_SPATK_EV, NULL);
    spDefEv = Pokemon_GetMonData(mon, MON_DATA_SPDEF_EV, NULL);
    if (Pokemon_GetMonData(mon, MON_DATA_SPECIES, NULL) != SPECIES_SHEDINJA && GetItemAttr_PreloadedItemData(itemData, ITEMATTR_HP_EV_UP)) {
...
#define sp54 stack_data[0]
    ...
    sp58 = sp54 = Pokemon_GetMonData(mon, MON_DATA_STATUS, NULL);
    ...
    if (sp54 != sp58) {
        Pokemon_SetData(mon, MON_DATA_STATUS, &sp58);
        hadEffect = TRUE;
    }

    sp54 = Pokemon_GetMonData(mon, MON_DATA_HP, NULL);
    sp58 = Pokemon_GetMonData(mon, MON_DATA_MAX_HP, NULL);
    if ((GetItemAttr_PreloadedItemData(itemData, ITEMATTR_REVIVE) || GetItemAttr_PreloadedItemData(itemData, ITEMATTR_REVIVE_ALL)) && GetItemAttr_PreloadedItemData(itemData, ITEMATTR_LEVEL_UP)) {
        if (sp54 == 0) {
    ...
    sp5C = Pokemon_GetMonData(mon, MON_DATA_LEVEL, NULL);
    if (GetItemAttr_PreloadedItemData(itemData, ITEMATTR_LEVEL_UP)) {
        if (sp5C < MAX_LEVEL) {
            Pokemon_AddData(mon, MON_DATA_EXPERIENCE, Pokemon_CalcExpToNextLevel(mon));
            Pokemon_CalcMonLevelAndStats(mon);
            if (sp54 == 0) {
                sp60 = Pokemon_GetMonData(mon, MON_DATA_MAX_HP, NULL);
    ...
    sp54 = Pokemon_GetMonData(mon, MON_DATA_HP_EV, NULL);
    sp58 = Pokemon_GetMonData(mon, MON_DATA_ATK_EV, NULL);
    sp5C = Pokemon_GetMonData(mon, MON_DATA_DEF_EV, NULL);
    sp60 = Pokemon_GetMonData(mon, MON_DATA_SPEED_EV, NULL);
    sp64 = Pokemon_GetMonData(mon, MON_DATA_SPATK_EV, NULL);
    sp68 = Pokemon_GetMonData(mon, MON_DATA_SPDEF_EV, NULL);
    if (Pokemon_GetMonData(mon, MON_DATA_SPECIES, NULL) != SPECIES_SHEDINJA && GetItemAttr_PreloadedItemData(itemData, ITEMATTR_HP_EV_UP)) {
    ...
    sp54 = Pokemon_GetMonData(mon, MON_DATA_FRIENDSHIP, NULL);
    if (sp54 < 100) {
...
BOOL MonMoveCanRestorePP(Pokemon *mon, int moveIdx) {
    u16 moveID = Pokemon_GetMonData(mon, MON_DATA_MOVE1 + moveIdx, NULL);
    if (moveID == MOVE_NONE) {
        return FALSE;
    }

    u8 pp = Pokemon_GetMonData(mon, MON_DATA_MOVE1_PP + moveIdx, NULL);
    u8 ppUp = Pokemon_GetMonData(mon, MON_DATA_MOVE1_PP_UPS + moveIdx, NULL);
    return (u8)(pp < GetMoveMaxPP(moveID, ppUp));
}

BOOL MonMoveRestorePP(Pokemon *mon, int moveIdx, int ppRestore) {
    u16 move_id = Pokemon_GetMonData(mon, MON_DATA_MOVE1 + moveIdx, NULL);
    if (move_id == MOVE_NONE) {
        return FALSE;
    }

    int ppAttr = MON_DATA_MOVE1_PP + moveIdx;
    u8 pp = Pokemon_GetMonData(mon, ppAttr, NULL);
    int ppUpAttr = MON_DATA_MOVE1_PP_UPS + moveIdx;
    u8 maxPp = GetMoveMaxPP(move_id, Pokemon_GetMonData(mon, ppUpAttr, NULL));
    if (pp < maxPp) {
...
BOOL BoostMonMovePpUpBy(Pokemon *mon, int moveIdx, int nPpUp) {
    ...
    int ppUpAttr = MON_DATA_MOVE1_PP_UPS + moveIdx;
    u8 ppUp = Pokemon_GetMonData(mon, ppUpAttr, NULL);
    if (ppUp == 3) {
        return FALSE;
    }

    move = Pokemon_GetMonData(mon, MON_DATA_MOVE1 + moveIdx, NULL);
    if (GetMoveMaxPP(move, 0) < 5) {
        return FALSE;
    }

    int ppAttr = MON_DATA_MOVE1_PP + moveIdx;
    pp = Pokemon_GetMonData(mon, ppAttr, NULL);
...
BOOL CanItemModFriendship(Pokemon *mon, ItemData *itemData) {
    s32 friendship = Pokemon_GetMonData(mon, MON_DATA_FRIENDSHIP, NULL);
    if (friendship >= 255) {
        return FALSE;
    }
...
BOOL DoItemFriendshipMod(Pokemon *mon, s32 friendship, s32 mod, u16 location, enum HeapID heapID) {
    ...
    if (mod > 0) {
        if (Pokemon_GetMonData(mon, MON_DATA_POKEBALL, NULL) == ITEM_LUXURY_BALL) {
            mod++;
        }

        if (location == Pokemon_GetMonData(mon, MON_DATA_EGG_LOCATION, NULL)) {
            mod++;
        }

        if (GetItemAttr(Pokemon_GetMonData(mon, MON_DATA_HELD_ITEM, NULL), ITEMATTR_HOLD_EFFECT, heapID) == HOLD_EFFECT_FRIENDSHIP_UP) {
            mod = mod * 150 / 100;
        }
    }
    ...
    Pokemon_SetData(mon, MON_DATA_FRIENDSHIP, &mod);
    return TRUE;
}
...
void HealParty(Party *party) {
    ...
    for (i = 0; i < partyCount; i++) {
        ...
        if (Pokemon_GetMonData(mon, MON_DATA_SPECIES_EXISTS, NULL)) {
            u32 sp8 = Pokemon_GetMonData(mon, MON_DATA_MAX_HP, NULL);
            Pokemon_SetData(mon, MON_DATA_HP, &sp8);

            sp8 = 0;
            Pokemon_SetData(mon, MON_DATA_STATUS, &sp8);
