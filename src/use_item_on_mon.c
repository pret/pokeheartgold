...
BOOL CanUseItemOnPokemon(Pokemon *mon, u16 itemID, s32 moveIdx, enum HeapID heapID) {
    ...
    if (GetItemAttr_PreloadedItemData(itemData, ITEMATTR_LEVEL_UP) && Pokemon_GetData(mon, MON_DATA_LEVEL, NULL) < MAX_MON_LEVEL) {
        Heap_Free(itemData);
        return TRUE;
    }
...
BOOL MonMoveCanRestorePP(Pokemon *mon, int moveIdx) {
    ...
    u8 pp = Pokemon_GetData(mon, MON_DATA_MOVE1_PP + moveIdx, NULL);
...
BOOL DoItemFriendshipMod(Pokemon *mon, s32 friendship, s32 mod, u16 location, enum HeapID heapID) {
    ...
    if (mod > 0) {
        ...
        if (location == Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL)) {
            mod++;
        }
