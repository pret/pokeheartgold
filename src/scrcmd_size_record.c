...
static u32 GetMonSizeHash(Pokemon *mon) {
    ...
    pid_lo = Pokemon_GetMonData(mon, MON_DATA_PERSONALITY, NULL);
    hpIv_lo = Pokemon_GetMonData(mon, MON_DATA_HP_IV, NULL) & 0xF;
    atkIv_lo = Pokemon_GetMonData(mon, MON_DATA_ATK_IV, NULL) & 0xF;
    defIv_lo = Pokemon_GetMonData(mon, MON_DATA_DEF_IV, NULL) & 0xF;
    spdIv_lo = Pokemon_GetMonData(mon, MON_DATA_SPEED_IV, NULL) & 0xF;
    spAtkIv_lo = Pokemon_GetMonData(mon, MON_DATA_SPATK_IV, NULL) & 0xF;
    spDefIv_lo = Pokemon_GetMonData(mon, MON_DATA_SPDEF_IV, NULL) & 0xF;
...
BOOL ScrCmd_BufferMonSize(ScriptContext *ctx) {
    ...
    slot = ScriptGetVar(ctx);
    mon = Party_GetMonByIndex(SaveArray_Party_Get(fieldSystem->saveData), slot);
    FormatSizeRecord(fieldSystem, idx0, idx1, Pokemon_GetMonData(mon, MON_DATA_SPECIES, NULL), GetMonSizeHash(mon));
    return FALSE;
}
