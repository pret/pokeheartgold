...
static void TrainerHouse_SetNames(TrainerHouseSet *set) {
    ...
    MsgData *messageData = NewMsgDataFromNarc(MSGDATA_LOAD_LAZY, NARC_msgdata_msg, NARC_msg_msg_0726_bin, HEAP_ID_FIELDMAP);
    ...
    for (s32 i = 0; i < PARTY_SIZE; i++) {
        ...
        String *name = GetSpeciesName(trainerHouseMon->species, HEAP_ID_FIELDMAP);
        GF_ASSERT(name);
        if (name) {
...
static BattleSetup *TrainerHouse_NewBattleSetup(FieldSystem *fieldSystem, TrainerHouseSet *set) {
    s32 i;
    BattleSetup *setup = BattleSetup_New(HEAP_ID_FIELDMAP, BATTLE_TYPE_TRAINER | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_13);
    ...
    Pokemon *mon = AllocMonZeroed(HEAP_ID_FIELDMAP);
...
static void TrainerHouse_InitBattleSetup(BattleSetup *setup, TrainerHouseSet *set, u32 battlerId) {
    ...
    Pokemon *tempMon = AllocMonZeroed(HEAP_ID_FIELDMAP);
