...
static void AlphPuzzle_PrintConfirmQuitText(AlphPuzzleData *data, int a1, int a2, u8 textFrameDelay) {
    ...
    if (textFrameDelay == 0) {
        ...
        data->textPrinterId = AddTextPrinterParameterized2(&data->window[1], 1, data->confirmQuitText[a1], 0, 0, textFrameDelay, MAKE_TEXT_COLOR(1, 2, 15), NULL);
    }
...
static void AlphPuzzle_PrintHintText(AlphPuzzleData *data) {
    ...
    AddTextPrinterParameterized2(&data->window[2], 4, data->hintText[data->puzzleIndex], 0, 0, 0xFF, MAKE_TEXT_COLOR(2, 1, 0), NULL);
