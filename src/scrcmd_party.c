...
BOOL ScrCmd_MonHasMove(ScriptContext *ctx) {
    ...
    *hasMove = FALSE;
    ...
    if (Pokemon_GetData(mon, MON_DATA_MOVE1, NULL) == move || Pokemon_GetData(mon, MON_DATA_MOVE2, NULL) == move || Pokemon_GetData(mon, MON_DATA_MOVE3, NULL) == move || Pokemon_GetData(mon, MON_DATA_MOVE4, NULL) == move) {
        *hasMove = TRUE;
    }
...
BOOL ScrCmd_PartyCountMonsAtOrBelowLevel(ScriptContext *ctx) {
    ...
    for (i = 0, count = 0, *levelCount = 0; i < partyCount; i++) {
        Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(fieldSystem->saveData), i);
        if (!Pokemon_GetData(mon, MON_DATA_IS_EGG, NULL) && Pokemon_GetData(mon, MON_DATA_LEVEL, NULL) <= level) {
            count++;
        }
    }
...
BOOL ScrCmd_MonAddFriendship(ScriptContext *ctx) {
    ...
    if (friendshipModifier != 0) {
        if (Pokemon_GetData(mon, MON_DATA_POKEBALL, NULL) == BALL_LUXURY) {
            friendshipModifier++;
        }

        if (map == Pokemon_GetData(mon, MON_DATA_EGG_MET_LOCATION, NULL)) {
            friendshipModifier++;
        }

        if (GetItemAttr(Pokemon_GetData(mon, MON_DATA_HELD_ITEM, NULL), ITEMATTR_HOLD_EFFECT, HEAP_ID_FIELD) == HOLD_EFFECT_FRIENDSHIP_UP) {
...
BOOL ScrCmd_MonGetContestValue(ScriptContext *ctx) {
    ...
    Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(ctx->fieldSystem->saveData), slot);
    *contestValue = Pokemon_GetData(mon, contestAttribute + MON_DATA_COOL, NULL);
...
BOOL ScrCmd_GetMonTypes(ScriptContext *ctx) {
    ...
    Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(fieldSystem->saveData), slot);
    *type1 = Pokemon_GetData(mon, MON_DATA_TYPE_1, NULL);
    *type2 = Pokemon_GetData(mon, MON_DATA_TYPE_2, NULL);
...
BOOL ScrCmd_CountAliveMons(ScriptContext *ctx) {
    ...
    for (i = 0, count = 0; i < partyCount; i++) {
        if (i != slot) {
            Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(fieldSystem->saveData), i);
            if (!Pokemon_GetData(mon, MON_DATA_IS_EGG, NULL) && Pokemon_GetData(mon, MON_DATA_HP, NULL) != 0) {
                count++;
            }
        }
    }
...
BOOL ScrCmd_MonGetMove(ScriptContext *ctx) {
    ...
    Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(fieldSystem->saveData), pokemonSlot);
    *move = Pokemon_GetData(mon, moveSlot + MON_DATA_MOVE1, NULL);
...
BOOL ScrCmd_PlayerHasSpecies(ScriptContext *ctx) {
    ...
    for (i = 0, *playerHasSpecies = FALSE; i < partyCount; i++) {
        Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(fieldSystem->saveData), i);
        if (!Pokemon_GetData(mon, MON_DATA_IS_EGG, NULL) && (u16)Pokemon_GetData(mon, MON_DATA_SPECIES, NULL) == species) {
...
BOOL ScrCmd_MonGetRibbonCount(ScriptContext *ctx) {
    ...
    for (i = 0, count = 0; i < RIBBON_MAX; i++) {
        if (Pokemon_GetData(mon, GetRibbonAttr(i, RIBBONDAT_MONDATNO), NULL)) {
            count++;
        }
    }
    *ribbons = count;
...
BOOL ScrCmd_GetPartyRibbonCount(ScriptContext *ctx) {
    ...
    for (i = 0, count = 0; i < RIBBON_MAX; i++) {
        for (j = 0; j < partyCount; j++) {
            ...
            if (!Pokemon_GetData(mon, MON_DATA_IS_EGG, NULL) && Pokemon_GetData(mon, GetRibbonAttr(i, RIBBONDAT_MONDATNO), NULL)) {
                count++;
                break;
            }
        }
...
BOOL ScrCmd_MonHasRibbon(ScriptContext *ctx) {
    ...
    Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(ctx->fieldSystem->saveData), slot);
    *hasRibbon = Pokemon_GetData(mon, GetRibbonAttr(ribbon, RIBBONDAT_MONDATNO), NULL);
...
BOOL ScrCmd_GetPartySlotWithFatefulEncounter(ScriptContext *ctx) {
    ...
    for (i = 0, *slot = 255; i < partyCount; i++) {
        Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(fieldSystem->saveData), i);
        if (!Pokemon_GetData(mon, MON_DATA_IS_EGG, NULL)) {
            if ((u16)Pokemon_GetData(mon, MON_DATA_SPECIES, NULL) == species && Pokemon_GetData(mon, MON_DATA_FATEFUL_ENCOUNTER, NULL) == TRUE) {
                *slot = i;
                break;
            }
        }
...
BOOL ScrCmd_MonAddContestValue(ScriptContext *ctx) {
    ...
    if (Pokemon_GetData(mon, MON_DATA_SHEEN, NULL) == 255) {
        return FALSE;
    }

    contestAttribute += MON_DATA_COOL;
    contestValue = Pokemon_GetData(mon, contestAttribute, NULL);
