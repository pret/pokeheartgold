...
BOOL ScrCmd_MonAddFriendship(ScriptContext *ctx) {
    ...
    if (friendshipModifier != 0) {
        ...
        if (map == Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL)) {
            friendshipModifier++;
        }

        if (GetItemAttr(Pokemon_GetData(mon, MON_DATA_HELD_ITEM, NULL), ITEMATTR_HOLD_EFFECT, HEAP_ID_FIELD2) == HOLD_EFFECT_FRIENDSHIP_UP) {
