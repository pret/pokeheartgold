...
BOOL ScrCmd_GiveMon(ScriptContext *ctx) {
    ...
    *retPtr = GiveMon(HEAP_ID_FIELDMAP, fieldSystem->saveData, species, level, form, ability, heldItem, map, 24);
...
BOOL ScrCmd_MonAddFriendship(ScriptContext *ctx) {
    ...
    if (friendshipModifier != 0) {
        ...
        if (GetItemAttr(GetMonData(mon, MON_DATA_HELD_ITEM, NULL), ITEMATTR_HOLD_EFFECT, HEAP_ID_FIELDMAP) == HOLD_EFFECT_FRIENDSHIP_UP) {
...
BOOL ScrCmd_KenyaCheck(ScriptContext *ctx) {
    ...
    Mail *mail = Mail_New(HEAP_ID_FIELDMAP);
