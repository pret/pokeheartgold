...
static void MGGive_Mon(FieldSystem *fieldSys, MysteryGiftData *unused) {
    ...
    int eggMetLocation = Pokemon_GetMonData(pokemon, MON_DATA_EGG_LOCATION, NULL);
    int personality = Pokemon_GetMonData(pokemon, MON_DATA_PERSONALITY, NULL);
    int otid = Pokemon_GetMonData(pokemon, MON_DATA_OT_ID, NULL);
    ...
    if (Pokemon_GetMonData(pokemon, MON_DATA_RED_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_RED)] = srcRibbons[0];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_GREEN_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_GREEN)] = srcRibbons[1];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_BLUE_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_BLUE)] = srcRibbons[2];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_FESTIVAL_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_FESTIVAL)] = srcRibbons[3];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_CARNIVAL_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_CARNIVAL)] = srcRibbons[4];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_CLASSIC_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_CLASSIC)] = srcRibbons[5];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_PREMIER_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_PREMIER)] = srcRibbons[6];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_MARINE_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_HOENN_MARINE)] = srcRibbons[7];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_LAND_RIBBON, NULL)) {
        ribbons->ribbons[GetSpecialRibbonNo(RIBBON_HOENN_LAND)] = srcRibbons[8];
    }
    if (Pokemon_GetMonData(pokemon, MON_DATA_SKY_RIBBON, NULL)) {
    ...
    if (Pokemon_GetMonData(pokemon, MON_DATA_SPECIES, NULL) == SPECIES_ARCEUS && Pokemon_GetMonData(pokemon, MON_DATA_FATEFUL_ENCOUNTER, NULL) == TRUE && !Save_VarsFlags_GetVar404C(vars_flags)) {
