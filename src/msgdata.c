...
void GetSpeciesNameIntoArray(u16 species, u32 heap_id, u16 * dest) {
    MSGDATA * msgData = NewMsgDataFromNarc(1, NARC_msgdata_msg, 237, heap_id);
...
STRING * GetMoveName(u32 move, HeapID heapno) {
    MSGDATA * msgData = NewMsgDataFromNarc(MSGDATA_LOAD_LAZY, NARC_msgdata_msg, 750, heapno);
    STRING * ret;
    if (msgData != NULL) {
...
STRING * GetSpeciesName(u16 species, HeapID heap_id) {
    STRING * ret;
    MSGDATA * msgData = NewMsgDataFromNarc(MSGDATA_LOAD_LAZY, NARC_msgdata_msg, 237, heap_id);
    if (msgData != NULL) {
