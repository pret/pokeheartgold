...
void NPCTrade_MakeAndGiveLoanMon(FieldSystem *fieldSystem, NpcTradeNum tradeno, u8 level, u16 mapno) {
    ...
    trade_dat = GfGfxLoader_LoadFromNarc(NARC_a_1_1_2, tradeno, FALSE, HEAP_ID_FIELDMAP, TRUE);
    _CreateTradeMon(mon, trade_dat, level, (NpcTradeNum)tradeno, mapno, 7, HEAP_ID_FIELDMAP);
    ...
    if (tradeno == NPC_TRADE_KENYA_SPEAROW) {
        ...
        name = _GetNpcTradeName(HEAP_ID_FIELDMAP, NPC_TRADE_OT_NUM(tradeno));
...
Mail *NPCTrade_MakeKenyaMail(void) {
    ...
    mon = AllocMonZeroed(HEAP_ID_FIELDMAP);
    trade_dat = GfGfxLoader_LoadFromNarc(NARC_a_1_1_2, 7, FALSE, HEAP_ID_FIELDMAP, TRUE);
    _CreateTradeMon(mon, trade_dat, 20, NPC_TRADE_KENYA_SPEAROW, MAP_ROUTE_35_GOLDENROD_GATEHOUSE, 7, HEAP_ID_FIELDMAP);
    name = _GetNpcTradeName(HEAP_ID_FIELDMAP, NPC_TRADE_OT_NUM(NPC_TRADE_KENYA_SPEAROW));
    mailno = ItemToMailId(trade_dat->heldItem);
    mail = CreateKenyaMail(mon, mailno, trade_dat->gender, name, trade_dat->otId);
    String_Delete(name);
    FreeToHeap(trade_dat);
