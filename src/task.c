...
static TaskManager *Task_New(FieldSystem *fieldSystem, TaskFunc taskFunc, void *env) {
    ...
    taskman = AllocFromHeapAtEnd((enum HeapID)32, sizeof(TaskManager));
    taskman->prev = NULL;
    ...
    taskman->unk1C = AllocFromHeapAtEnd((enum HeapID)32, sizeof(TaskManagerUnkSub1C));
    return taskman;
}
...
void CallApplicationAsTask(TaskManager *taskManager, const OverlayManagerTemplate *template, void *work) {
    ...
    env = AllocFromHeapAtEnd((enum HeapID)32, sizeof(struct UnkTaskEnv));
    env->state = 0;
