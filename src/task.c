...
static TaskManager *Task_New(FieldSystem *fieldSystem, TaskFunc taskFunc, void *env) {
    ...
    taskman = Heap_AllocAtEnd((HeapID)32, sizeof(TaskManager));
    taskman->prev = NULL;
    ...
    taskman->unk1C = Heap_AllocAtEnd((HeapID)32, sizeof(TaskManagerUnkSub1C));
    return taskman;
}
...
void CallApplicationAsTask(TaskManager *taskManager, const OverlayManagerTemplate *template, void *work) {
    ...
    env = Heap_AllocAtEnd((HeapID)32, sizeof(struct UnkTaskEnv));
    env->state = 0;
