...
BOOL sub_0206D494(FieldSystem *fieldSystem) {
    ...
    if (unk2) {
        UnkStruct_0206D494 *unkStruct = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(UnkStruct_0206D494));
...
BOOL MonIsInGameTradePoke(Pokemon *mon, NpcTradeNum tradeNum) {
    NPCTrade *trade = GfGfxLoader_LoadFromNarc(NARC_a_1_1_2, tradeNum, FALSE, HEAP_ID_FIELDMAP, TRUE);
...
static BOOL MonIsInGameTradePokeInternal(Pokemon *mon, NPCTrade *trade, NpcTradeNum tradeNum) {
    ...
    MsgData *messageData = NewMsgDataFromNarc(MSGDATA_LOAD_DIRECT, NARC_msgdata_msg, NARC_msg_msg_0200_bin, HEAP_ID_FIELDMAP);
    String *monNickname = String_New(12, HEAP_ID_FIELDMAP);
    ...
    String *monOtName = String_New(8, HEAP_ID_FIELDMAP);
