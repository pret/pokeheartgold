...
BOOL MonIsFromTogepiEgg(Pokemon *mon, SaveData *saveData) {
    ...
    u8 byte = Pokemon_GetMonData(mon, MON_DATA_OT_GENDER, NULL);
    if (byte != PlayerProfile_GetTrainerGender(profile)) {
        return FALSE;
    }
    byte = Pokemon_GetMonData(mon, MON_DATA_LANGUAGE, NULL);
    if (byte != gGameLanguage) {
        return FALSE;
    }
    byte = Pokemon_GetMonData(mon, MON_DATA_MET_GAME, NULL);
    if (byte != gGameVersion) {
        return FALSE;
    }
    SaveMisc_GetTogepiPersonalityGender(misc, &word, &byte);
    if (word != Pokemon_GetMonData(mon, MON_DATA_PERSONALITY, NULL)) {
        return FALSE;
    }
    if (byte != Pokemon_GetMonData(mon, MON_DATA_GENDER, NULL)) {
        return FALSE;
    }
    return TRUE;
}
...
static BOOL MonIsInGameTradePokeInternal(Pokemon *mon, NPCTrade *trade, NpcTradeNum tradeNum) {
    ...
    u32 otId = Pokemon_GetMonData(mon, MON_DATA_OT_ID, NULL);
    if (otId != trade->otId) {
        return FALSE;
    }
    u32 pid = Pokemon_GetMonData(mon, MON_DATA_PERSONALITY, NULL);
    if (pid != trade->pid) {
        return FALSE;
    }
    u8 gender = Pokemon_GetMonData(mon, MON_DATA_OT_GENDER, NULL);
    if (gender != trade->gender) {
        return FALSE;
    }
    u8 language = Pokemon_GetMonData(mon, MON_DATA_LANGUAGE, NULL);
    if (language != trade->language) {
        return FALSE;
    }
    u8 version = Pokemon_GetMonData(mon, MON_DATA_MET_GAME, NULL);
    ...
    Pokemon_GetMonData(mon, MON_DATA_NICKNAME_STRING, monNickname);
    ...
    Pokemon_GetMonData(mon, MON_DATA_OT_NAME_STRING, monOtName);
