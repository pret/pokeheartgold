...
static void ListMenuPrint(struct ListMenu *list, String *str, u8 x, u8 y) {
    if (str != NULL) {
        if (list->overrideEnabled) {
            AddTextPrinterParameterizedWithColorAndSpacing(list->template.window, list->fontId, str, x, y, 0xFF, MAKE_TEXT_COLOR(list->cursorPal, list->cursorShadowPal, list->fillValue), list->lettersSpacing, 0, NULL);
        } else {
            AddTextPrinterParameterizedWithColorAndSpacing(list->template.window, list->template.fontId, str, x, y, 0xFF, MAKE_TEXT_COLOR(list->template.cursorPal, list->template.cursorShadowPal, list->template.fillValue), list->template.lettersSpacing, 0, NULL);
        }
    }
}
