...
BOOL ScrCmd_394(ScriptContext *ctx) {
    ...
    *runningAppData = LearnForgetMove_LaunchApp(HEAP_ID_FIELD_TASK, ctx->fieldSystem, var0, 0);
...
BOOL ScrCmd_466(ScriptContext *ctx) {
    ...
    u16 *eligibleMoves = MoveRelearner_GetEligibleLevelUpMoves(mon, HEAP_ID_FIELD_TASK);
...
static void StartMoveRelearner(ScriptContext *ctx, int type, Pokemon *mon, u16 *eligibleMoves) {
    ...
    MoveRelearnerArgs *moveRelearner = MoveRelearner_New(HEAP_ID_FIELD_TASK);
...
BOOL ScrCmd_MoveTutor(ScriptContext *ctx) {
    ...
    Pokemon *mon = Party_GetMonByIndex(SaveArray_Party_Get(ctx->fieldSystem->saveData), slot);
    u16 *eligibleMoves = AllocFromHeap(HEAP_ID_FIELD_TASK, 2 * sizeof(u16));
