...
void BugContest_RestoreParty_RetrieveCaughtPokemon(BugContest *bugContest) {
    ...
    if (bugContest->caught_poke) {
        if (bugContest->party_cur_num >= PARTY_SIZE) {
            PCStorage_PlaceMonInFirstEmptySlotInAnyBox(SaveArray_PCStorage_Get(bugContest->saveData), Pokemon_GetBoxPokemon(bugContest->mon));
        } else {
...
u16 BugContest_JudgePlayerMon(BugContest *bugContest, Pokemon *mon) {
    ...
    score += Pokemon_GetData(mon, MON_DATA_HP, NULL) * 100 / Pokemon_GetData(mon, MON_DATA_MAX_HP, NULL);
    return score;
}
