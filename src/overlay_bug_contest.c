...
void BugContest_Judge(BugContest *bugContest) {
    ...
    player->data.species = Pokemon_GetMonData(bugContest->mon, MON_DATA_SPECIES, NULL);
...
BOOL BugContest_BufferCaughtMonNick(BugContest *bugContest, MessageFormat *msgFmt, u8 slot) {
    ...
    Pokemon_GetMonData(bugContest->mon, MON_DATA_NICKNAME_STRING, string);
...
void BugContest_RestoreParty_RetrieveCaughtPokemon(BugContest *bugContest) {
    ...
    if (bugContest->caught_poke) {
        if (bugContest->party_cur_num >= PARTY_SIZE) {
            PCStorage_PlaceMonInFirstEmptySlotInAnyBox(SaveArray_PCStorage_Get(bugContest->saveData), Pokemon_GetBox(bugContest->mon));
        } else {
...
u16 BugContest_JudgePlayerMon(BugContest *bugContest, Pokemon *mon) {
    ...
    score += Pokemon_GetMonData(mon, MON_DATA_LEVEL, NULL) * 100 / bugmon->lvlmax;
    stat_total = 0;
    for (i = 0; i < NUM_STATS; i++) {
        stat_total += Pokemon_GetMonData(mon, MON_DATA_HP_IV + i, NULL);
    }
    score += stat_total * 100 / (31 * NUM_STATS);
    score += Pokemon_GetMonData(mon, MON_DATA_HP, NULL) * 100 / Pokemon_GetMonData(mon, MON_DATA_MAX_HP, NULL);
    return score;
}
