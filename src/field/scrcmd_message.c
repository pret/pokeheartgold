...
BOOL ScrCmd_NonNpcMsgExtern(ScriptContext *ctx) {
    ...
    MsgData *messageData = NewMsgDataFromNarc(MSGDATA_LOAD_LAZY, NARC_msgdata_msg, fileId, HEAP_ID_FIELD_TASK);
...
BOOL ScrCmd_443(ScriptContext *ctx) {
    ...
    if (r4[0] == 0xFFFF) {
        MsgData *messageData = NewMsgDataFromNarc(MSGDATA_LOAD_LAZY, NARC_msgdata_msg, 723, HEAP_ID_FIELD_TASK);
...
static void ovFieldMain_GetFormattedECMessage(MessageBox *messageBox, u16 messageBank, u16 messageNum, u16 word1, u16 word2) {
    ...
    String *string = MailMsg_GetExpandedString(&mailMessage, HEAP_ID_FIELD_TASK);
