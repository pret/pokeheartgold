...
void FieldSystem_BeginSinjohCutsceneTask(FieldSystem *fieldSystem) {
    SinjohCutsceneData *data = AllocFromHeapAtEnd(HEAP_ID_FIELD2, sizeof(SinjohCutsceneData));
...
static void SinjohCutscene_LoadResources(SinjohCutsceneData *data) {
    ...
    GF_ExpHeap_FndInitAllocator(&data->alloc, HEAP_ID_FIELD1, 32);
...
void FieldSystem_BeginSinjohGetEggCutsceneTask(FieldSystem *fieldSystem, u8 whichLegend) {
    SinjohGetEggCutsceneData *data = AllocFromHeapAtEnd(HEAP_ID_FIELD2, sizeof(SinjohGetEggCutsceneData));
