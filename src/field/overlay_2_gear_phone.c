...
BOOL ov02_02251F20(GearPhoneRingManager *gearPhone) {
    ...
    PhoneBook *phoneBook = AllocAndReadPhoneBook(HEAP_ID_FIELD);
...
static u32 ov02_02251FDC(GearPhoneRingManager *gearPhone, PhoneBook *phoneBook, u32 mapId) {
    ...
    contact = SavePokegear_AllocAndCopyPhonebook(gearPhone->pokegearData, HEAP_ID_FIELD);
    u8 *ptr = AllocFromHeapAtEnd(HEAP_ID_FIELD, slot);
...
static u8 ov02_02252218(GearPhoneRingManager *gearPhone, PhoneBook *phoneBook, u32 mapId) {
    u8 *ptr = AllocFromHeapAtEnd(HEAP_ID_FIELD, 13);
