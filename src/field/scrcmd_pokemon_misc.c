...
BOOL ScrCmd_StatJudge(ScriptContext *ctx) {
    ...
    ivList[0] = Pokemon_GetData(mon, MON_DATA_HP_IV, 0);
    ivList[1] = Pokemon_GetData(mon, MON_DATA_ATK_IV, 0);
    ivList[2] = Pokemon_GetData(mon, MON_DATA_DEF_IV, 0);
    ivList[3] = Pokemon_GetData(mon, MON_DATA_SPEED_IV, 0);
    ivList[4] = Pokemon_GetData(mon, MON_DATA_SPATK_IV, 0);
    ivList[5] = Pokemon_GetData(mon, MON_DATA_SPDEF_IV, 0);
...
BOOL ScrCmd_CommSanitizeParty(ScriptContext *ctx) {
    ...
    *success = FALSE;
    ...
    for (i = 0; i < partyCount; i++) {
        ...
        heldItems[i] = Pokemon_GetData(mon, MON_DATA_HELD_ITEM, 0);
...
BOOL ScrCmd_CountTranformedRotomsInParty(ScriptContext *ctx) {
    ...
    *firstIndex = 255;
    ...
    for (i = 0; i < partyCount; i++) {
        ...
        species = Pokemon_GetData(mon, MON_DATA_SPECIES, 0);
        form = Pokemon_GetData(mon, MON_DATA_FORM, 0);
        isEgg = Pokemon_GetData(mon, MON_DATA_IS_EGG, 0);
...
BOOL ScrCmd_GetHiddenPowerType(ScriptContext *ctx) {
    ...
    u16 species = Pokemon_GetData(mon, MON_DATA_SPECIES, 0);

    if (Pokemon_GetData(mon, MON_DATA_IS_EGG, 0) == FALSE) {
        switch (species) {
...
static void GetHiddenPowerPowerType(Pokemon *mon, s32 *power, s32 *type) {
    int hpIv = Pokemon_GetData(mon, MON_DATA_HP_IV, 0);
    int atkIv = Pokemon_GetData(mon, MON_DATA_ATK_IV, 0);
    int defIv = Pokemon_GetData(mon, MON_DATA_DEF_IV, 0);
    int spdIv = Pokemon_GetData(mon, MON_DATA_SPEED_IV, 0);
    int spatkIv = Pokemon_GetData(mon, MON_DATA_SPATK_IV, 0);
    int spdefIv = Pokemon_GetData(mon, MON_DATA_SPDEF_IV, 0);
    if (power) {
...
BOOL ScrCmd_SetFavoriteMon(ScriptContext *ctx) {
    ...
    u32 form = Pokemon_GetData(mon, MON_DATA_FORM, 0);
    u32 isEgg = Pokemon_GetData(mon, MON_DATA_IS_EGG, 0);
...
BOOL ScrCmd_GiveTogepiEgg(ScriptContext *ctx) {
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) {
        if (Pokemon_GetData(mon, MON_DATA_MOVE1 + i, 0) == MOVE_NONE) {
            break;
        }
    }
    ...
    SaveMisc_SetTogepiPersonalityGender(Save_Misc_Get(fieldSystem->saveData), Pokemon_GetData(mon, MON_DATA_PERSONALITY, 0), Pokemon_GetData(mon, MON_DATA_GENDER, 0));
...
BOOL ScrCmd_GiveSpikyEarPichu(ScriptContext *ctx) {
    ...
    Pokemon_InitWith(mon, SPECIES_PICHU, 30, 0x20, 1, unkA, 1, trId);
...
static u32 MonGetShinyLeafCount(Pokemon *mon) {
    ...
    for (c = 0; c < 5; c++) {
        shinyLeafCount += Pokemon_GetData(mon, MON_DATA_SHINY_LEAF_A + c, NULL);
    }
    if (shinyLeafCount == 5 && Pokemon_GetData(mon, MON_DATA_SHINY_LEAF_CROWN, NULL) != 0) {
        shinyLeafCount = 6;
    }
...
BOOL ScrCmd_CheckKyogreGroudonInParty(ScriptContext *ctx) {
    ...
    for (c = 0; c < partyCount; c++) {
        ...
        if (!Pokemon_GetData(mon, MON_DATA_IS_EGG, NULL)
            ...
            && !Pokemon_GetData(mon, MON_DATA_EGG_LOCATION, NULL)) {
...
BOOL ScrCmd_JudgeBugContest(ScriptContext *ctx) {
    ...
    *prize = bugContest->prize;
    ...
    if (bugContest->caught_poke == 0) {
        ...
        *species = Pokemon_GetData(bugContest->mon, MON_DATA_SPECIES, NULL);
    }
