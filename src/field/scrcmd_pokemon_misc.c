...
BOOL ScrCmd_699(ScriptContext *ctx) {
    ...
    while (MapObjectManager_IterObjects(mapObjectManager, &curObj, &unkVar, MAPOBJECTFLAG_ACTIVE) == TRUE) {
...
BOOL ScrCmd_700(ScriptContext *ctx) {
    ...
    while (MapObjectManager_IterObjects(mapObjectManager, &curObj, &index, MAPOBJECTFLAG_ACTIVE) == TRUE) {
        if (curObj == playerObj) continue;
