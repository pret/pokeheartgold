...
void FieldSystem_TryHeadbuttEncounter(FieldSystem *fieldSystem, u16 *varPointer) {
    TaskData_TryHeadbuttEncounter *didHeadbuttStartBattle = AllocFromHeapAtEnd(HEAP_ID_FIELDMAP, sizeof(didHeadbuttStartBattle));
...
static BOOL Task_TryHeadbuttEncounter(TaskManager *taskManager) {
    ...
    headbuttTable = AllocAtEndAndReadWholeNarcMemberByIdPair(NARC_arc_headbutt, fieldSystem->location->mapId, HEAP_ID_FIELDMAP);
